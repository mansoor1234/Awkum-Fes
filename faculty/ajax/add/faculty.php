...
<?php } ?>
<?php 
if (isset($_POST['getFaculty']) && isset($_POST['courseID'])) {
  extract($_POST);
  $semesterID=$_POST['semesterID'];
  $get_faculty_data=$conn->prepare("SELECT *
   FROM faculty WHERE semester_id=:semesterID AND campus_id=:campus AND program_id=:program AND department_id=:department AND course_id=:course");
  $get_faculty_data->bindParam(":semesterID",$semesterID);
  $get_faculty_data->bindParam(":campus",$campus);
  $get_faculty_data->bindParam(":program",$program);
  $get_faculty_data->bindParam(":department",$department);
  $get_faculty_data->bindParam(":course",$courseID);
  $get_faculty_data->execute();
  $rows1=$get_faculty_data->fetchAll(PDO::FETCH_ASSOC);
  $count=$get_faculty_data->rowCount();
     echo "<option value='0'>Choose Faculty</option>";
     for ($i=0; $i < $count; $i++) {?>
        <option value="<?php echo $rows1[$i]['srno'];?>"><?php echo $rows1[$i]['name']; ?></option>
       <?php
     }
}
 ?>
