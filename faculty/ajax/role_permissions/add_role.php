...
}if(!empty($campus) || empty($campus)){
$result=fetchFields('campus',6);
}if(!empty($dept) || empty($dept)){
$result=fetchFields('dept',7);
}if(!empty($program) || empty($program)){
$result=fetchFields('program',8);
}if(!empty($semester) || empty($semester)){
$result=fetchFields('semester',9);
}if(!empty($faculty) || empty($faculty)){
$result=fetchFields('faculty',10);
}if(!empty($course) || empty($course)){
$result=fetchFields('course',11);
}if(!empty($reports) || empty($reports)){
$result=fetchFields('reports',12);
}
}
if($result){
echo "true";
